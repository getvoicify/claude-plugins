...
| `status.py` | Reports epic completion, Project-field drift, live PR watches (`watches[]`: child, PR, `quiet_s`, `last_activity`), and (with `--sweep-plan`) which worktrees are safe to remove — read-only, never mutates | `python3 epic/scripts/status.py --epic <epic#> --repo <owner/name> [--sweep-plan]` | `0` always (the report itself carries the finding); `2` `gh` error — never returns `1` |
